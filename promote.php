...
if (empty($_GET['encode']))
{
    ...
    header("Location: promote.php?encode=$string\n");
...
else
{
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        ...
        "WHERE g.is_delete = 0 AND b.promote > 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
   ...
   // echo b.promote/100;
    /* 查询商品 */
     $where = 'WHERE 1 ';
    $where .= " AND ((b.promote > 0 AND  b.end_date >= '$now_date') OR  ( g.promote_end_date  >= '$now_date_time'   AND g.market_price < 1.00 )) ";
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price,b.promote,b.brand_name, b.end_date, ".
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "IFNULL(mp.user_price, g.shop_price * b.promote /100  * '$_SESSION[discount]') AS brand_shop_price, ".
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                     "LEFT JOIN " . $GLOBALS['ecs']->table('brand') . " AS b ".
                    "ON b.brand_id = g.brand_id ".
            "$where AND g.is_delete = 0 AND g.market_price !=1.00 AND g.is_on_sale = 1  AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
            "ORDER BY $sort $order";
    ...
    while ($row = $db->FetchRow($res))
    {
        ...
        if(local_date('Y-m-d', $row['promote_end_date']+60*60*24) >= $now_date  && $row['market_price'] < 1.00 ){
        ...
        $arr[$row['goods_id']]['shop_price']    = $shop_price;
    ...
    /* 分页 */
    $url_format = "promote.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
    if (!empty($intromode))
    {
    ...
    $pager = get_pager('promote.php', $pager['search'], $count, $page, $size);
