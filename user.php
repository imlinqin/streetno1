...
elseif ($action == 'order_list')
{
    ...
    $consignee = empty($_REQUEST['consignee']) ? 0 :$_REQUEST['consignee'];
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    //$orders = get_user_orders($user_id, $pager['size'], $pager['start']);
    $orders = get_user_orders2($user_id, $pager['size'], $pager['start'],$consignee);
...
elseif ($action == 'order_detail')
{
    ...
    //by lin
    $order_sn = $_GET['order_sn'] ? $_GET['order_sn'] : 0;
    //echo $order_id;
     /*  //by lin 查找单号 */
    ...
    {
        $goods_list[$key]['market_price'] = price_format($value['market_price'], true);
        $goods_list[$key]['goods_price']  = price_format($value['goods_price'], true);
        $goods_list[$key]['subtotal']     = price_format($value['subtotal'], true);
    }
...
elseif ($action == 'account_detail')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $row['user_money'] = price_format(abs($row['user_money']), true);
        $row['frozen_money'] = price_format(abs($row['frozen_money']), true);
    ...
    //模板赋值
    $smarty->assign('surplus_amount', price_format($surplus_amount, true));
...
elseif ($action == 'act_account')
{
    ...
    else
    {
        ...
        $smarty->assign('pay_fee', price_format($payment_info['pay_fee'], true));
        $smarty->assign('amount',  price_format($amount, true));
        $smarty->assign('order',   $order);
        $smarty->display('user_transaction.dwt');
    }
}
...
elseif ($action == 'pay')
{
    ...
    if (!empty($payment_info))
    {
        ...
        $smarty->assign('order',   $order);
        $smarty->assign('pay_fee', price_format($payment_info['pay_fee'], true));
        $smarty->assign('amount',  price_format($order['surplus_amount'], true));
