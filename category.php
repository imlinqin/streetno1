...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if (1)
   // if (is_file('themes/default/images/category_banner/'.$cat_id.'.jpg'))
	{
		$categories_pic=1;
	}
	else
    ...
    if(intval($_REQUEST['class'])==46){
        $extcat = 45;
    }
     if(intval($_REQUEST['class'])==45){
        $extcat = 46;
    }
    elseif($cat_id==46){
        $extcat = 45;
   ...
   // echo '<pre>';
    //print_r(get_categories_tree(40)[40]); ;


    $position = assign_ur_here($cat_id, $brand_name);
    $smarty->assign('page_title',       $position['title']);    // 页面标题
    $smarty->assign('ur_here',          $position['ur_here']);  // 当前位置
    $smarty->assign('categories_pic',   $categories_pic);
    ...
    $smarty->assign('class',            intval($_REQUEST['class']));
    ...
    $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order,$extcat);
    if($display == 'grid')
    {
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order,$extcat=0)
{
   ...
   // $extcat = $extcat == 45  ? 45 :46;
    if ($extcat > 0)
    {
        $where .=  $extcat == 45 ? " AND NOT c.cat_id = 45" : " AND NOT c.cat_id = 46";
    }

    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, c.cat_id, b.promote, b.end_date, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price,IFNULL(mp.user_price, g.shop_price *  b.promote /100 * '$_SESSION[discount]') AS brand_shop_price,
                IFNULL(mp.user_price, g.shop_price *  g.market_price * '$_SESSION[discount]') AS good_price,
                 g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ,b.brand_name ,g.brand_id ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
		' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
          'LEFT JOIN ' . $GLOBALS['ecs']->table('goods_cat') . ' AS c ON c.goods_id = g.goods_id ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else
        {
          ...
          $now_date = date('Y-m-d');
            ...
            if($row['promote_end_date'] >= $now_date_time  && $row['market_price'] < 1.00 ){
            $shop_price = price_format($row['good_price']);
            $promote = $row['market_price'] * 100;
            }
             elseif ($row['market_price'] == 1.00 ) {
                $shop_price = '';
                $promote = 100 ;
            }
            
            elseif ($row['end_date'] >= $now_date && $row['promote'] ) {
                $shop_price = price_format($row['brand_shop_price']);
                $promote = $row['promote'] ;
            }
            else {
        ...
        $arr[$row['goods_id']]['promote']     = $promote;
        $arr[$row['goods_id']]['brand_shop_price'] = $shop_price;
        $arr[$row['goods_id']]['good_price'] = $shop_price;
