...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $smarty->assign('data_dir',        DATA_DIR);       // 数据目录
		...
		$smarty->assign('ad_ad',recommend_pic('0'));       // 弹出广告
	$smarty->assign('ad_top',recommend_pic('1'));       // 弹出广告
	$smarty->assign('ad_center',recommend_pic('2'));       // 弹出广告
	$smarty->assign('ad_bottom',recommend_pic('3'));       // 弹出广告
 ...
 !empty($_GET['best']) ?  $smarty->display('best.dwt', $cache_id) :  $smarty->display('index.dwt', $cache_id);
...
/*首页弹出广告 by lin*/
function recommend_pic($ad_pic)
{
	$array=array();
	$rows=$GLOBALS['db']->getAll("SELECT * FROM  ".$GLOBALS['ecs']->table("ad_custom")." WHERE ad_status = 1 and ad_type='$ad_pic' order by ad_id desc limit 0,4");

	
	foreach($rows as $row)
	{
		
		$array[$row['ad_id']]['ad_id']=$row['ad_id'];
		$array[$row['ad_id']]['ad_type']=$row['ad_type'];
		$array[$row['ad_id']]['content']=$row['content'];
		$array[$row['ad_id']]['url']=$row['url'];
		$array[$row['ad_id']]['ad_name']=$row['ad_name'];
...
//  echo '<pre>';
// // print_r($study);
//  print_r(parseXmlFile('./images/index_img.xml'));
