...
elseif ($_REQUEST['act'] == 'list')
{
 ...
 if ($_REQUEST['korea_start_date'] and $_REQUEST['korea_end_date'])
	...
	$delivery = array(
             ...
             'korea_start_date' => local_date('Y-m-d', local_strtotime('-10 day')),
...
elseif ($_REQUEST['act'] == 'list2')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_list2.htm');
}
...
elseif ($_REQUEST['act'] == 'query2')
{
    ...
    $sort_flag  = sort_flag($order_list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('order_list2.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    //$order['formated_total_fee'] = sprintf($GLOBALS['_CFG']['formated_total_fee'], round(str_replace('￥','',$order['formated_total_fee']))) ;
    $order['formated_total_fee'] = sprintf($GLOBALS['_CFG']['currency_format'], round(str_replace('￥','',$order['formated_total_fee']))) ;
    $order['formated_goods_amount'] = sprintf($GLOBALS['_CFG']['currency_format'], round(str_replace('￥','',$order['formated_goods_amount']))) ;
   // $order['formated_goods_amount'] = round($order['formated_goods_amount'])
    ...
    $order['region'] = $db->getOne($sql);
     ...
     $order['ishongkong'] = strstr($order['region'], '香港特别'); 
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, g.goods_img 
...
elseif ($_REQUEST['act'] == 'delivery_email')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('delivery_email.htm');
}
...
elseif ($_REQUEST['act'] == 'delivery_email_query')
{
   ...
   $delivery = array(
    ...
    $sort_flag = sort_flag($result['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('delivery_email.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($cfg == '1')
        {
            ...
            if (!send_mail($user['user_name'], $user['email'], $tpl['template_subject'], $content, $tpl['is_html']))
            {
                $msg = $_LANG['send_mail_fail'];
            }
           
           // send_mail('linqin', '381530891@qq.com', $order['email'], $content,$tpl['is_html']);
        }
       // send_mail('linqin', '381530891@qq.com', 'streentno1', '我是内容');
        /* 如果需要，发短信 */
        if (1)
        {
            ...
            $user['user_name'];
            $sms = new sms();
            $sms->send($user['mobile_phone'], sprintf($GLOBALS['_LANG']['order_shipped_sms'], $order['order_sn'],
                local_date($GLOBALS['_LANG']['sms_time_format']), 'NO1'), 0);
        }
...
elseif ($_REQUEST['act'] == 'ordergoodstatus')
{
    ...
    //if ($exc->edit("is_hot = '$is_hot', last_update=" .gmtime(), $rec_id))
	// if($db->query("UPDATE " . $ecs->table('order_goods') . " SET status = '$delivering' , delivery_time = '" . gmtime() . "' WHERE rec_id ='$rec_id'"))
    // {
    //     clear_cache_files();
    ...
    if($delivering == 4 )
    {
       $db->query("UPDATE " . $ecs->table('order_goods') . " SET status_paizi = '$delivering'   WHERE rec_id ='$rec_id'");
    }
    

      $db->query("UPDATE " . $ecs->table('order_goods') . " SET status = '$delivering' , delivery_time = '" . gmtime() . "'  WHERE rec_id ='$rec_id'");
...
elseif ($_REQUEST['act'] == 'delivery_sendemail')
{
   ...
   $content = '<p>亲爱的'.$user['user_name'].'----收货人('.$consignee.')。你好！<br />
    ...
    send_mail($user['user_name'], $user['email'], 'streetno1发货通知', $content,1);
     ...
     /* 返回 */
        sys_msg('邮件发送成功', 0, array(array('href'=>'order.php?act=delivery_list' , 'text' => $_LANG['return_list'])));
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['export2']))
	{
        ...
        ->setCategory("");
         $PHPExcel->setActiveSheetIndex(0);
        $PHPExcel->getActiveSheet()->setTitle("发货列表");
        //填入表头主标题
        $PHPExcel->getActiveSheet()->setCellValue('A1', $_CFG['shop_name'].'发货列表');
        //填入表头副标题
        $PHPExcel->getActiveSheet()->setCellValue('A2', '操作者：'.$_SESSION['admin_name'].' 导出日期：'.date('Y-m-d',time()).' 地址：'.$_CFG['shop_address'].' 电话：'.$_CFG['service_phone']);
        //合并表头单元格
		...
		$PHPExcel->getActiveSheet()->getStyle('A2')->getFont()->setName('宋体');
        $PHPExcel->getActiveSheet()->getStyle('A2')->getFont()->setSize(14);
        //$PHPExcel->getActiveSheet()->getStyle('A3:AE3')->getFont()->setSize(12);
       // $PHPExcel->getActiveSheet()->getStyle('A3:AE3')->getFont()->setSize(10);
       // $PHPExcel->getActiveSheet()->getStyle('A3:AE3')->getFill()->getStartColor()->setARGB('FF808080');
        $PHPExcel->getActiveSheet()->getStyle( 'A3:AE3')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);  
        $PHPExcel->getActiveSheet()->getStyle( 'A3:AE3')->getFill()->getStartColor()->setARGB('80ee9922');  
       ...
       //表格宽度
        $PHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(15);//订单编号
        $PHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(15);//下单时间
        $PHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(15);//付款时间
        $PHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(15);//发货时间
        $PHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(15);//发货单号
        $PHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(15);//支付方式
        $PHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(60);//配送方式
        $PHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(10);//配送费用
        $PHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(10);//收件人
        $PHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(15);//收货地址
        $PHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth(15);//电话
        $PHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(15);//手机
        $PHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(15);//邮箱
        $PHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(15);//货号
        $PHPExcel->getActiveSheet()->getColumnDimension('O')->setWidth(70);//商品名称
        $PHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(15);//属性
        $PHPExcel->getActiveSheet()->getColumnDimension('Q')->setWidth(15);//价格
        $PHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(15);//数量
        $PHPExcel->getActiveSheet()->getColumnDimension('S')->setWidth(15);//小计
        $PHPExcel->getActiveSheet()->getColumnDimension('T')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('U')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('V')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('W')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('X')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('Y')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('Z')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AA')->setWidth(15);//数量
        $PHPExcel->getActiveSheet()->getColumnDimension('AB')->setWidth(15);//小计
        $PHPExcel->getActiveSheet()->getColumnDimension('AC')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AD')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AE')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AF')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AG')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AH')->setWidth(15);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AI')->setWidth(15);//应付款金额

        //表格标题
         $PHPExcel->getActiveSheet()->setCellValue('A3', 'SF Waybill Number');
        $PHPExcel->getActiveSheet()->setCellValue('B3', 'Shipment Reerence No.');
        $PHPExcel->getActiveSheet()->setCellValue('C3', 'CustomerAccountNo.');
        $PHPExcel->getActiveSheet()->setCellValue('D3', 'Company Name');
        $PHPExcel->getActiveSheet()->setCellValue('E3', 'Contact Name');
        $PHPExcel->getActiveSheet()->setCellValue('F3', 'Tel');
        $PHPExcel->getActiveSheet()->setCellValue('G3', 'Address 1');
       // $PHPExcel->getActiveSheet()->setCellValue('E3', '价格');
        $PHPExcel->getActiveSheet()->setCellValue('H3', 'Address 2');
         $PHPExcel->getActiveSheet()->setCellValue('I3', 'Address 3');
         $PHPExcel->getActiveSheet()->setCellValue('J3', 'Country');
          $PHPExcel->getActiveSheet()->setCellValue('K3', 'Postal Code');
           $PHPExcel->getActiveSheet()->setCellValue('L3', 'Company Name');
          $PHPExcel->getActiveSheet()->setCellValue('M3', 'ContactName');
        $PHPExcel->getActiveSheet()->setCellValue('N3', 'Tel');
       $PHPExcel->getActiveSheet()->setCellValue('O3', 'Address 1');
        $PHPExcel->getActiveSheet()->setCellValue('P3', 'Address 2');
         $PHPExcel->getActiveSheet()->setCellValue('Q3', 'Address 3');
         $PHPExcel->getActiveSheet()->setCellValue('R3', 'Country');
         $PHPExcel->getActiveSheet()->setCellValue('S3', 'Post code');
         $PHPExcel->getActiveSheet()->setCellValue('T3', 'Currency');
         $PHPExcel->getActiveSheet()->setCellValue('U3', 'Brand name 1');
         $PHPExcel->getActiveSheet()->setCellValue('V3', 'item 1');
          $PHPExcel->getActiveSheet()->setCellValue('W3', 'Quantity 1');
           $PHPExcel->getActiveSheet()->setCellValue('X3', 'Unit Price 1');

            $PHPExcel->getActiveSheet()->setCellValue('Z3', 'Shipment');
             $PHPExcel->getActiveSheet()->setCellValue('AA3', 'Total Package');
              $PHPExcel->getActiveSheet()->setCellValue('AB3', 'Total Actual Weight');
               $PHPExcel->getActiveSheet()->setCellValue('AC3', 'Payment Method');
                $PHPExcel->getActiveSheet()->setCellValue('AD3', 'AccountNo.');
                 $PHPExcel->getActiveSheet()->setCellValue('AE3', 'Third Party District Code');
        ...
        foreach ($order_sn_list as $order_sn) {
            ...
            /* 取得订单商品 */
            $sql        = "SELECT o.*, g.goods_number AS storage, g.supply_price, c.cat_name, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name " . "FROM " . $ecs->table('order_goods') . " AS o " . "LEFT JOIN " . $ecs->table('goods') . " AS g ON o.goods_id = g.goods_id " . "LEFT JOIN " . $ecs->table('brand') . " AS b ON g.brand_id = b.brand_id " . "LEFT JOIN " . $ecs->table('category') . " AS c ON g.cat_id = c.cat_id " . "WHERE o.order_id = '$order[order_id]' ";
            $res        = $db->query($sql);
            $shuliang   = 0;
            $chanpin    = $hang;
            while ($row = $db->fetchRow($res)) {
                ...
                // $PHPExcel->getActiveSheet()->setCellValue('S' . $chanpin, $row['formated_subtotal']);
                 ...
                 $PHPExcel->getActiveSheet()->setCellValue('A' . $chanpin, $order['invoice_no']." ");
                $PHPExcel->getActiveSheet()->setCellValue('B' . $chanpin, $order['order_sn']." ");
                 $PHPExcel->getActiveSheet()->setCellValue('C' . $chanpin, ' 0825000291');
                $PHPExcel->getActiveSheet()->setCellValue('D' . $chanpin, 'flowave');
                $PHPExcel->getActiveSheet()->setCellValue('E' . $chanpin, 'Song ha woong');
                $PHPExcel->getActiveSheet()->setCellValue('F' . $chanpin, '070-4078-4856');
			//	$PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, $row['goods_price']);
                 $PHPExcel->getActiveSheet()->setCellValue('G' . $chanpin, '201,no61,anamdong2ga, Seongbuk-gu, Seoul, Republic of Korea');
                $PHPExcel->getActiveSheet()->setCellValue('H' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('I' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('J' . $chanpin, 'Korea');
                $PHPExcel->getActiveSheet()->setCellValue('K' . $chanpin, ' 02850');
                $PHPExcel->getActiveSheet()->setCellValue('L' . $chanpin, $order['consignee']);
                $PHPExcel->getActiveSheet()->setCellValue('M' . $chanpin, $order['consignee']);
                $PHPExcel->getActiveSheet()->setCellValue('N' . $chanpin, $order['tel']);
                 $PHPExcel->getActiveSheet()->setCellValue('O' . $chanpin, str_replace(" ","",$order['region']).$order['address']);
                  $PHPExcel->getActiveSheet()->setCellValue('P' . $chanpin, '');
                  $PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, '');
                  $PHPExcel->getActiveSheet()->setCellValue('R' . $chanpin, 'China');
                  $PHPExcel->getActiveSheet()->setCellValue('S' . $chanpin, $order['zipcode']);
                  $PHPExcel->getActiveSheet()->setCellValue('T' . $chanpin, 'won');
                  $PHPExcel->getActiveSheet()->setCellValue('U' . $chanpin, $row['brand_name']);
                   $PHPExcel->getActiveSheet()->setCellValue('V' . $chanpin, $row['cat_name']);
                    $PHPExcel->getActiveSheet()->setCellValue('W' . $chanpin, $row['goods_number']);
                     $PHPExcel->getActiveSheet()->setCellValue('X' . $chanpin, $row['supply_price']);
                      $PHPExcel->getActiveSheet()->setCellValue('Y' . $chanpin, '');

                       $PHPExcel->getActiveSheet()->setCellValue('Z' . $chanpin, 'SE');
                        $PHPExcel->getActiveSheet()->setCellValue('AA' . $chanpin, '1');
                         $PHPExcel->getActiveSheet()->setCellValue('AB' . $chanpin, '0.5');
                          $PHPExcel->getActiveSheet()->setCellValue('AC' . $chanpin, 'Shipper (Credit A/C)');
                           $PHPExcel->getActiveSheet()->setCellValue('AD' . $chanpin, ' 0825000291');
                         $PHPExcel->getActiveSheet()->setCellValue('AE' . $chanpin, '');
		...
		//设置单元格边框
		$PHPExcel->getActiveSheet()->getStyle('A1:AE'.$hang)->applyFromArray($styleArray);
		//设置自动换行
		$PHPExcel->getActiveSheet()->getStyle('A4:AE'.$hang)->getAlignment()->setWrapText(true);
		//设置字体大小
		$PHPExcel->getActiveSheet()->getStyle('A4:AE'.$hang)->getFont()->setSize(12);
		//垂直居中
		$PHPExcel->getActiveSheet()->getStyle('A1:AE'.$hang)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		//水平居中
		$PHPExcel->getActiveSheet()->getStyle('A1:AE'.$hang)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    elseif (isset($_POST['export3']))
	{
        ...
        ->setCategory("");
         $PHPExcel->setActiveSheetIndex(0);
        $PHPExcel->getActiveSheet()->setTitle("圆通发货列表");
        //填入表头主标题
        $PHPExcel->getActiveSheet()->setCellValue('A1', $_CFG['shop_name'].'圆通发货列表');
        //填入表头副标题
        $PHPExcel->getActiveSheet()->setCellValue('A2', '操作者：'.$_SESSION['admin_name'].' 导出日期：'.date('Y-m-d',time()).' 地址：'.$_CFG['shop_address'].' 电话：'.$_CFG['service_phone']);
        //合并表头单元格
        $PHPExcel->getActiveSheet()->mergeCells('A1:AU1');
        $PHPExcel->getActiveSheet()->mergeCells('A2:AU2');
       ...
       // $PHPExcel->getActiveSheet()->getStyle('A3:AE3')->getFont()->setSize(10);
       // $PHPExcel->getActiveSheet()->getStyle('A3:AE3')->getFill()->getStartColor()->setARGB('FF808080');
        $PHPExcel->getActiveSheet()->getStyle( 'A3:AV3')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);  
        $PHPExcel->getActiveSheet()->getStyle( 'A3:AV3')->getFill()->getStartColor()->setARGB('80ee9922');  
       ...
       //表格宽度
       // $PHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(15);//单号
        $PHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(15);//下单时间
        $PHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(15);//发货日期
        $PHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(15);//发货单号
        $PHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(15);//发货地区
        $PHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(15);//P/O No
        $PHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(60);//KN
        $PHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(10);//Item
        $PHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(10);//颜色
        $PHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(15);//码数
        $PHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(15);//发货人名
        $PHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth(15);//发货人电话
        $PHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(15);//发货人地址
        $PHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(15);//收件人英文名
        $PHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(70);//收件人电话
        $PHPExcel->getActiveSheet()->getColumnDimension('O')->setWidth(15);//收件人手机
        $PHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(15);//收件人国家
        $PHPExcel->getActiveSheet()->getColumnDimension('Q')->setWidth(15);//包装数量
        $PHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(15);//重量
        $PHPExcel->getActiveSheet()->getColumnDimension('S')->setWidth(15);//总共价格USD
        $PHPExcel->getActiveSheet()->getColumnDimension('T')->setWidth(15);//牌子名
        $PHPExcel->getActiveSheet()->getColumnDimension('U')->setWidth(15);//品名
        $PHPExcel->getActiveSheet()->getColumnDimension('V')->setWidth(15);//货数量
        $PHPExcel->getActiveSheet()->getColumnDimension('W')->setWidth(15);//X列
        $PHPExcel->getActiveSheet()->getColumnDimension('X')->setWidth(15);//个单价
        $PHPExcel->getActiveSheet()->getColumnDimension('Y')->setWidth(15);//收件人中文名
        $PHPExcel->getActiveSheet()->getColumnDimension('Z')->setWidth(15);//省
        $PHPExcel->getActiveSheet()->getColumnDimension('AA')->setWidth(15);//城市
        $PHPExcel->getActiveSheet()->getColumnDimension('AB')->setWidth(20);//区
        $PHPExcel->getActiveSheet()->getColumnDimension('AC')->setWidth(15);//地址
        $PHPExcel->getActiveSheet()->getColumnDimension('AD')->setWidth(15);//AE列
        $PHPExcel->getActiveSheet()->getColumnDimension('AE')->setWidth(15);//中文品名
        $PHPExcel->getActiveSheet()->getColumnDimension('AF')->setWidth(15);//AG列
        $PHPExcel->getActiveSheet()->getColumnDimension('AG')->setWidth(15);//AH列
        $PHPExcel->getActiveSheet()->getColumnDimension('AH')->setWidth(15);//AI列
        $PHPExcel->getActiveSheet()->getColumnDimension('AI')->setWidth(15);//AJ列
        $PHPExcel->getActiveSheet()->getColumnDimension('AJ')->setWidth(15);//总共价格（元）
        $PHPExcel->getActiveSheet()->getColumnDimension('AK')->setWidth(20);//单个价格（元）
        $PHPExcel->getActiveSheet()->getColumnDimension('AL')->setWidth(15);//AM列 Item
        $PHPExcel->getActiveSheet()->getColumnDimension('AM')->setWidth(15);//AN列
        $PHPExcel->getActiveSheet()->getColumnDimension('AN')->setWidth(15);//中文品名 Item 中文
        $PHPExcel->getActiveSheet()->getColumnDimension('AO')->setWidth(15);//AP列
        $PHPExcel->getActiveSheet()->getColumnDimension('AP')->setWidth(15);//AQ列
        $PHPExcel->getActiveSheet()->getColumnDimension('AQ')->setWidth(15);//AR列
        $PHPExcel->getActiveSheet()->getColumnDimension('AR')->setWidth(15);//AS列
        $PHPExcel->getActiveSheet()->getColumnDimension('AS')->setWidth(15);//AT列
        $PHPExcel->getActiveSheet()->getColumnDimension('AT')->setWidth(15);//AU列
        $PHPExcel->getActiveSheet()->getColumnDimension('AU')->setWidth(15);//AV列
         $PHPExcel->getActiveSheet()->getColumnDimension('AV')->setWidth(15);//AV列

        //表格标题
        // $PHPExcel->getActiveSheet()->setCellValue('A3', '');
        $PHPExcel->getActiveSheet()->setCellValue('A3', 'NO');
        $PHPExcel->getActiveSheet()->setCellValue('B3', '발송날짜');// 发货日期
        $PHPExcel->getActiveSheet()->setCellValue('C3', '배송번호'); //发货单号
        $PHPExcel->getActiveSheet()->setCellValue('D3', '배송지역');//发货地区
        $PHPExcel->getActiveSheet()->setCellValue('E3', 'P/o NO');
        $PHPExcel->getActiveSheet()->setCellValue('F3', '중국주문번호'); 
        $PHPExcel->getActiveSheet()->setCellValue('G3', '제품명');// Item
        $PHPExcel->getActiveSheet()->setCellValue('H3', '색상'); //颜色
        $PHPExcel->getActiveSheet()->setCellValue('J3', '사이즈');// 码数
        $PHPExcel->getActiveSheet()->setCellValue('J3', '발송인 이름'); // 发货人名
        $PHPExcel->getActiveSheet()->setCellValue('K3', '발송인 전화번호'); // 发货人电话
        $PHPExcel->getActiveSheet()->setCellValue('L3', '발송인 주소'); // 发货人地址
        $PHPExcel->getActiveSheet()->setCellValue('M3', '수취인영문이름');//收件人英文名
        $PHPExcel->getActiveSheet()->setCellValue('N3', '수취인 일반전화'); //收件人电话
        $PHPExcel->getActiveSheet()->setCellValue('O3', '수취인 핸드폰번호'); //收件人手机
        $PHPExcel->getActiveSheet()->setCellValue('P3', '수취인 국가'); //收件国家
        $PHPExcel->getActiveSheet()->setCellValue('Q3', '박스수량'); //包装数量
        $PHPExcel->getActiveSheet()->setCellValue('R3', '총무게'); // 重量
        $PHPExcel->getActiveSheet()->setCellValue('S3', '물품 총 가격 US$'); // 总共价格usd
        $PHPExcel->getActiveSheet()->setCellValue('T3', 'B브랜드 명'); //牌子名 
        $PHPExcel->getActiveSheet()->setCellValue('U3', '물품명'); //品名
        $PHPExcel->getActiveSheet()->setCellValue('V3', '물품갯수'); //货数量
        $PHPExcel->getActiveSheet()->setCellValue('W3', '개당 무게');
        $PHPExcel->getActiveSheet()->setCellValue('X3', '물품 개당 가격 US$'); //个单价
        $PHPExcel->getActiveSheet()->setCellValue('Y3', '수취인중문이름'); //收件人中文名
        $PHPExcel->getActiveSheet()->setCellValue('Z3', '성'); //省
        $PHPExcel->getActiveSheet()->setCellValue('AA3', '시'); //城市
        $PHPExcel->getActiveSheet()->setCellValue('AB3', '구'); //区
        $PHPExcel->getActiveSheet()->setCellValue('AC3', '상세주소'); //地址
        $PHPExcel->getActiveSheet()->setCellValue('AD3', '배송정보');

        $PHPExcel->getActiveSheet()->setCellValue('AE3', '중문품명'); //中文品名 
        $PHPExcel->getActiveSheet()->setCellValue('AF3', '반송주소'); //
        $PHPExcel->getActiveSheet()->setCellValue('AG3', '배송형태'); 
        $PHPExcel->getActiveSheet()->setCellValue('AH3', '신분증번호');
        $PHPExcel->getActiveSheet()->setCellValue('AI3', '상품코드'); //
        $PHPExcel->getActiveSheet()->setCellValue('AJ3', '총가격(RMB)'); //重工价格（人民币）
        $PHPExcel->getActiveSheet()->setCellValue('AK3', '개당가격(RMB)'); //个单价（人民币）
        $PHPExcel->getActiveSheet()->setCellValue('AL3', '아이템영문'); //item
        $PHPExcel->getActiveSheet()->setCellValue('AM3', '영문혼용율'); //
        $PHPExcel->getActiveSheet()->setCellValue('AN3', '아이템중문'); //item中文名
        $PHPExcel->getActiveSheet()->setCellValue('AO3', '중문혼용율');
        $PHPExcel->getActiveSheet()->setCellValue('AP3', '성별'); //
        $PHPExcel->getActiveSheet()->setCellValue('AQ3', 'AREA CODE'); //
        $PHPExcel->getActiveSheet()->setCellValue('AR3', 'HAWB NO'); //
        $PHPExcel->getActiveSheet()->setCellValue('AS3', 'H B/L(통관)'); //
        $PHPExcel->getActiveSheet()->setCellValue('AT3', 'H B/L(택배)');
        $PHPExcel->getActiveSheet()->setCellValue('AU3', 'COMPANY CODE');
         $PHPExcel->getActiveSheet()->setCellValue('AV3', 'URL');
        $hang = 4; 
        foreach ($order_sn_list as $order_sn) {
            /* 取得订单信息 */
            $order = order_info(0, $order_sn);
            ...
            /* 取得区域名 */
            $sql             = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " . "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region , c.region_name AS country ,p.region_name AS province ,t.region_name  AS city " . "FROM " . $ecs->table('order_info') . " AS o " . "LEFT JOIN " . $ecs->table('region') . " AS c ON o.country = c.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS p ON o.province = p.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS t ON o.city = t.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS d ON o.district = d.region_id " . "WHERE o.order_id = '$order[order_id]'";
            $order['region'] = $db->getRow($sql);
            ...
            while ($row = $db->fetchRow($res)) {
                ...
                // $PHPExcel->getActiveSheet()->setCellValue('S' . $chanpin, $row['formated_subtotal']);
                 ...
                 //$PHPExcel->getActiveSheet()->setCellValue('A' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('A' . $chanpin, '');
                 $PHPExcel->getActiveSheet()->setCellValue('B' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('C' . $chanpin, $order['order_sn']." ");
                $PHPExcel->getActiveSheet()->setCellValue('D' . $chanpin, $order['region']['city']);
                $PHPExcel->getActiveSheet()->setCellValue('E' . $chanpin, "s".$order['order_sn']);
			//	$PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, $row['goods_price']);
                 $PHPExcel->getActiveSheet()->setCellValue('F' . $chanpin, $order['order_sn']." ");
                $PHPExcel->getActiveSheet()->setCellValue('G' . $chanpin, $row['cat_name']);
                $PHPExcel->getActiveSheet()->setCellValue('H' . $chanpin, $yanse);
                $PHPExcel->getActiveSheet()->setCellValue('I' . $chanpin, $mashu);
                $PHPExcel->getActiveSheet()->setCellValue('J' . $chanpin, '이상진');
                $PHPExcel->getActiveSheet()->setCellValue('K' . $chanpin, '82-010-8654-4856');
                $PHPExcel->getActiveSheet()->setCellValue('L' . $chanpin, '201,no61,anamdong2ga, Seongbuk-gu, Seoul, Republic of Korea');
                $PHPExcel->getActiveSheet()->setCellValue('M' . $chanpin, $order['consignee']);
                $PHPExcel->getActiveSheet()->setCellValue('N' . $chanpin, " ".$order['tel']);
                $PHPExcel->getActiveSheet()->setCellValue('O' . $chanpin, " ".$order['tel']);
                $PHPExcel->getActiveSheet()->setCellValue('P' . $chanpin, 'CHINA');
                $PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, '1');
                $PHPExcel->getActiveSheet()->setCellValue('R' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('S' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('T' . $chanpin, $row['brand_name']);
                $PHPExcel->getActiveSheet()->setCellValue('U' . $chanpin,$row['cat_name']);
                $PHPExcel->getActiveSheet()->setCellValue('V' . $chanpin, $row['goods_number']);
                $PHPExcel->getActiveSheet()->setCellValue('W' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('X' . $chanpin, $row['goods_price']*0.15);

                $PHPExcel->getActiveSheet()->setCellValue('Y' . $chanpin, $order['consignee']);
                $PHPExcel->getActiveSheet()->setCellValue('Z' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AA' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AB' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AC' . $chanpin, str_replace("中国","",str_replace(" ","",$order['region']['region']).$order['address']));
                $PHPExcel->getActiveSheet()->setCellValue('AD' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AE' . $chanpin, $row['cat_name']);
                $PHPExcel->getActiveSheet()->setCellValue('AF' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AG' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AH' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AI' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AJ' . $chanpin,  $row['goods_price'] * $row['goods_number']);
                $PHPExcel->getActiveSheet()->setCellValue('AK' . $chanpin, $row['goods_price']);
                $PHPExcel->getActiveSheet()->setCellValue('AL' . $chanpin, $row['cat_name']);
                $PHPExcel->getActiveSheet()->setCellValue('AM' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AN' . $chanpin, $row['cat_name']);
                $PHPExcel->getActiveSheet()->setCellValue('AO' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AP' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AQ' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AR' . $chanpin, "s".$order['order_sn']);
                $PHPExcel->getActiveSheet()->setCellValue('AS' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AT' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('AU' . $chanpin, 'ST');
                $PHPExcel->getActiveSheet()->setCellValue('AU' . $chanpin, 'ST');
                $PHPExcel->getActiveSheet()->setCellValue('AV' . $chanpin, "http://streetno1.com/goods.php?id=".$row['goods_id']);
		...
		//设置单元格边框
		$PHPExcel->getActiveSheet()->getStyle('A1:AV'.$hang)->applyFromArray($styleArray);
		//设置自动换行
		$PHPExcel->getActiveSheet()->getStyle('A4:AV'.$hang)->getAlignment()->setWrapText(true);
		//设置字体大小
		$PHPExcel->getActiveSheet()->getStyle('A4:AV'.$hang)->getFont()->setSize(12);
		//垂直居中
		$PHPExcel->getActiveSheet()->getStyle('A1:AV'.$hang)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		//水平居中
		$PHPExcel->getActiveSheet()->getStyle('A1:AV'.$hang)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    /* 导出订单功能结束 */
        ...
        /* 导出订单快递运费  ---  运费表*/
    elseif (isset($_REQUEST['downorder']))
	{
         ecs_header("Location: $url\n");
    ...
    elseif (isset($_REQUEST['delivery_export']))
	{
        ...
        // $PHPExcel->getActiveSheet()->setCellValue('K3', '电话');
        // $PHPExcel->getActiveSheet()->setCellValue('L3', '手机');
         $PHPExcel->getActiveSheet()->setCellValue('B3', '品牌名');
        $PHPExcel->getActiveSheet()->setCellValue('C3', '货号');
        $PHPExcel->getActiveSheet()->setCellValue('D3', '商品名称');
        $PHPExcel->getActiveSheet()->setCellValue('E3', '韩国名称');
        $PHPExcel->getActiveSheet()->setCellValue('F3', '颜色');
        $PHPExcel->getActiveSheet()->setCellValue('G3', '码数');
       // $PHPExcel->getActiveSheet()->setCellValue('E3', '价格');
        $PHPExcel->getActiveSheet()->setCellValue('H3', '数量');
       ...
       // $PHPExcel->getActiveSheet()->setCellValue('T3', '商品总金额');
        ...
        foreach ($rec_id_list as $rec_id) {
            ...
            // /* 取得订单商品 */
            // $sql        = "SELECT d.*, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name " . "FROM " . $ecs->table('delivery_goods') . " AS d " . "LEFT JOIN " . $ecs->table('delivery_order') . " AS o ON o.delivery_id = d.delivery_id " . "WHERE o.rec_id = '$order_sn' ";


              /* 取得发货单商品 */
            //  $sql = "SELECT d.* ,o.order_sn ,o.add_time , o.update_time ,o.delivery_id ,g.supply_price , b.discount    
            //       FROM " . $GLOBALS['ecs']->table('delivery_goods')  . " AS d ".
            //       "LEFT JOIN " . $GLOBALS['ecs']->table('delivery_order') . ' AS o ON o.delivery_id = d.delivery_id ' . 
            ...
            while ($row = $db->fetchRow($res)) {
                ...
                //     }
                // }
               // $yanse =  strpos(explode(" ",$row['goods_attr'])[0], 'COLOR');
                $row['promote'] = $row['promote'] > 100 ? 100 : $row['promote'];
               ...
               // $yanse =  $row['goods_attr'];
                ...
                if($row['status'] == 1 || $row['status_paizi'] == 1 || $row['order_status']!=5) {
                    $row['goods_number']= 0 ;
                }
            ...
            //    $row['newdiscount'] =  $row['discount']!=0 ? $row['discount'] :$row['bdiscount'];
               ...
               $row['newdiscount'] =  !empty($row['discount']) && $row['discount'] !=0 ? $row['discount'] :$row['bdiscount'];
                ...
                if($row['order_status']!=5 || $row['status'] ==1 || $row['status_paizi'] == 1){ //非发货的
                //设置填充的样式和背景色
                $PHPExcel->getActiveSheet()->getStyle('A' . $hang . ':M' . $hang)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                $PHPExcel->getActiveSheet()->getStyle('A' . $hang . ':M' . $hang)->getFill()->getStartColor()->setARGB('FFFFF68F');
                }
                //var_dump($order);die;
                //输出订单的商品，由于可能一个人购买多个商品，所以在这先输出了
                ...
                $PHPExcel->getActiveSheet()->setCellValue('D' . $chanpin, $row['goods_name']);
                $PHPExcel->getActiveSheet()->setCellValue('E' . $chanpin, $row['korea_name']);
                $PHPExcel->getActiveSheet()->setCellValue('F' . $chanpin, $yanse);
			//	$PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, $row['goods_price']);
                 $PHPExcel->getActiveSheet()->setCellValue('G' . $chanpin, trim($mashu));
                $PHPExcel->getActiveSheet()->setCellValue('H' . $chanpin, $row['goods_number']);
                ...
                $PHPExcel->getActiveSheet()->setCellValue('M' . $chanpin, ($row['promote']?$row['promote']:100) * 0.01*(100-$row['newdiscount']) * 0.01 * $row['supply_price'] * $row['goods_number']);
                //  $PHPExcel->getActiveSheet()->setCellValue('S' . $chanpin, $row['formated_subtotal']);
    ...
    /* 导出订单功能结束 */
        ...
        /* 货源导出订单功能开始 */
    elseif (isset($_REQUEST['delivery_export2']))
	{
        if (empty($_REQUEST['delivery_export2']))
        {
            sys_msg($_LANG['pls_select_order']);
        }
        ...
        //表格宽度
        $PHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(18);//订单编号
        $PHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(40);//下单时间
        $PHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(20);//付款时间
        $PHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(20);//发货时间
        $PHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(18);//发货单号
        $PHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);//支付方式
        ...
        $PHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(25);//邮箱
        $PHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(15);//货号
        $PHPExcel->getActiveSheet()->getColumnDimension('O')->setWidth(20);//商品名称
        $PHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(20);//属性
        $PHPExcel->getActiveSheet()->getColumnDimension('Q')->setWidth(20);//价格
        $PHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(20);//数量
        $PHPExcel->getActiveSheet()->getColumnDimension('S')->setWidth(20);//小计
        $PHPExcel->getActiveSheet()->getColumnDimension('T')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('U')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('V')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('W')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('X')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('Y')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('Z')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AA')->setWidth(20);//数量
        $PHPExcel->getActiveSheet()->getColumnDimension('AB')->setWidth(20);//小计
        $PHPExcel->getActiveSheet()->getColumnDimension('AC')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AD')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AE')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AF')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AG')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AH')->setWidth(20);//应付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('AI')->setWidth(20);//应付款金额
        ...
        foreach ($rec_id_list as $rec_id) {
           ...
           //  $order = order_info(0, $order_sn);
            ...
            //        "WHERE d.rec_id = '$rec_id' ";
                ...
                /* 取得发货单商品 */
             $sql = "SELECT d.* ,o.order_sn ,o.order_id,o.add_time ,o.consignee,o.address,o.tel,  b.brand_name ,b.brand_id , b.discount , b.promote , o.order_status , o.shipping_status ,g.korea_name ,g.supply_price    
                  FROM " . $GLOBALS['ecs']->table('order_goods')  . " AS d ".
                 // "LEFT JOIN " . $GLOBALS['ecs']->table('order_action') . ' AS a ON a.order_id = d.order_id ' . 
                  "LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . ' AS o ON o.order_id = d.order_id ' . 
                  "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = d.goods_id ' . 
           ...
           //  $order = order_info(0, $row['order_sn']);
            ...
            while ($row = $db->fetchRow($res)) {
               ...
               // $yanse =  $row['goods_attr'];
                ...
                // }
                       ...
                       /* 取得区域名 */
            $sqlr             = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " . "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " . "FROM " . $ecs->table('order_info') . " AS o " . "LEFT JOIN " . $ecs->table('region') . " AS c ON o.country = c.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS p ON o.province = p.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS t ON o.city = t.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS d ON o.district = d.region_id " . "WHERE o.order_id = '$row[order_id]'";
            $region = $db->getOne($sqlr);
                ...
                //输出订单的商品，由于可能一个人购买多个商品，所以在这先输出了
                $PHPExcel->getActiveSheet()->setCellValue('A' . $chanpin, '');
                $PHPExcel->getActiveSheet()->setCellValue('B' . $chanpin, $row['order_sn']);
                 $PHPExcel->getActiveSheet()->setCellValue('C' . $chanpin, ' 0825000291');
                $PHPExcel->getActiveSheet()->setCellValue('D' . $chanpin, 'flowave');
                $PHPExcel->getActiveSheet()->setCellValue('E' . $chanpin, 'Song ha woong');
                $PHPExcel->getActiveSheet()->setCellValue('F' . $chanpin, '070-4078-4856');
			//	$PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, $row['goods_price']);
                ...
                $PHPExcel->getActiveSheet()->setCellValue('J' . $chanpin, 'Korea');
                $PHPExcel->getActiveSheet()->setCellValue('K' . $chanpin, ' 02850');
                $PHPExcel->getActiveSheet()->setCellValue('L' . $chanpin, $row['consignee']);
                $PHPExcel->getActiveSheet()->setCellValue('M' . $chanpin, $row['consignee']);
                $PHPExcel->getActiveSheet()->setCellValue('N' . $chanpin, $row['tel']);
                 $PHPExcel->getActiveSheet()->setCellValue('O' . $chanpin, str_replace(" ","",$region).$row['address']);
                  $PHPExcel->getActiveSheet()->setCellValue('P' . $chanpin, '');
                  $PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, '');
                  $PHPExcel->getActiveSheet()->setCellValue('R' . $chanpin, 'China');
                  $PHPExcel->getActiveSheet()->setCellValue('S' . $chanpin, '');
                  $PHPExcel->getActiveSheet()->setCellValue('T' . $chanpin, 'won');
                  $PHPExcel->getActiveSheet()->setCellValue('U' . $chanpin, '');
                   $PHPExcel->getActiveSheet()->setCellValue('V' . $chanpin, '');
                    $PHPExcel->getActiveSheet()->setCellValue('W' . $chanpin, '');
                     $PHPExcel->getActiveSheet()->setCellValue('X' . $chanpin, '');
                      $PHPExcel->getActiveSheet()->setCellValue('Y' . $chanpin, '');
            ...
            for ($kk = $hang; $kk < ($hang + $shuliang); $kk++) {
                ...
                $PHPExcel->getActiveSheet()->mergeCells('T' . $hang . ':T' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('U' . $hang . ':U' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('V' . $hang . ':V' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('W' . $hang . ':W' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('X' . $hang . ':X' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('Y' . $hang . ':Y' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('Z' . $hang . ':Z' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('AA' . $hang . ':AA' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('AB' . $hang . ':AB' . $kk);
                 $PHPExcel->getActiveSheet()->mergeCells('AC' . $hang . ':AC' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('AD' . $hang . ':AD' . $kk);
                $PHPExcel->getActiveSheet()->mergeCells('AE' . $hang . ':AE' . $kk);
            }
          //  $PHPExcel->getActiveSheet()->setCellValue('A' . ($hang), $row['order_sn']." ");//加个空格，防止时间戳被转换
           // $PHPExcel->getActiveSheet()->setCellValue('B' . ($hang), $order['order_time']);
          //  $PHPExcel->getActiveSheet()->setCellValue('C' . ($hang), $order['pay_time']);
...
elseif ($_REQUEST['act']  == 'fahuoupload')
{
         if (!empty ( $_FILES['file_stu']['name'] )) 

            {
                    ...
                    print_r ('<pre>');
                     ...
                     $file_data = importExecl($tmp_file);
                     
                    $is_alimama = $file_data[1][A] == 'ETD' ? true : false;
                    $last_data = end($file_data);
                    print_r ($file_data);
                        /* 获取商品good id */
                       // $max_id = $db->getOne("SELECT MAX(order_id) + 1 FROM ".$ecs->table('order_info'));
                          /* 循环插入商品数据 */
                          if($is_alimama){
                              // print_r("<pre/>");
                               ...
                               foreach ($file_data as $key => $value) {
                                   if($value[A]){
                                          ...
                                          $sql = "SELECT order_id,user_id,shipping_status " .
           
                                        " FROM " . $GLOBALS['ecs']->table('order_info') . 
                                        "WHERE order_sn = '$value[A]'";
                                         $row = $GLOBALS['db']->getRow($sql);

                                        if($row[shipping_status]!=1){
                                         $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET invoice_no = '$value[B]', shipping_status = 1 " .
                                         "WHERE order_sn = '$value[A]'");
                                         $nowDate = gmtime(); 
                                         $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('delivery_order') . " SET invoice_no = '$value[B]', status = 0, " .
                                         "update_time = $nowDate ".
                                         "WHERE order_sn = '$value[A]'");

                                          $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('order_goods') . " SET  status = 2 " .
                                         "WHERE order_id = '$row[order_id]'");
                                        
                                       
                                         /* 发送邮件 */
                                       
                                        // $content = '亲爱的'.$user['user_name'] . '您的订单' . $value[A].'已于'.local_date($_CFG['date_format']).'发货，圆通发货单号为'.$value[B].'，感谢您对我们的支持。';
                                        //  send_mail($user['user_name'], $user['email'], 'streetno1发货通知', $content,0);
                                          ...
                                          /* 如果需要，发短信 */
                                            if (0)
                                            {
                                                include_once('../includes/cls_sms.php');
                                                 ...
                                                 $user = user_info($row['user_id']);
                                                $user['user_name'];
                                                $sms = new sms();
                                                $sms->send($user['mobile_phone'], sprintf($GLOBALS['_LANG']['order_shipped_sms'], $value[A],
                                                    local_date($GLOBALS['_LANG']['sms_time_format']), 'NO1'), 0);
                                            }
                                     }
     ...
     /* 显示提示信息，返回商品列表 */
   // $link[] = array('href' => 'cql.php?act=add', 'text' => '成功');
  //  sys_msg($_LANG['batch_upload_ok'], 0, $link);
   // die('成功!');
     ecs_header("Location: order.php?act=delivery_email\n");
        exit;
 ...
 * @return void
 */
function order_list($user_class=0)
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        $filter['fahuo'] = empty($_REQUEST['fahuo']) ? 0 : $_REQUEST['fahuo'];
         $filter['daohuo'] = empty($_REQUEST['daohuo']) ? 0 : $_REQUEST['daohuo'];
        ...
        $filter['end_time'] = empty($_REQUEST['end_time']) ? '' : (strpos($_REQUEST['end_time'], '-') > 0 ?  local_strtotime($_REQUEST['end_time']) : $_REQUEST['end_time']);
         ...
         $user_class = empty($_REQUEST['user_class']) ?  $user_class : trim($_REQUEST['user_class']) ;
        ...
        if($user_class ==2){
            $where .= " AND u.home_phone = 0 ";
        }
         if($user_class ==1){
            $where .= " AND u.home_phone = 1 ";
        }
    //     if (!$filter['order_sn'])
    //     {
	...
	// 		$where .= " AND a.log_time >='".(strtotime($filter['korea_start_date']))."' AND  a.log_time < '".(strtotime($filter['korea_end_date'])+60*60*24)."'";
    //     }
      ...
      elseif ($filter['fahuo']) {
                  ...
                  $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o ".
              " LEFT JOIN " .$GLOBALS['ecs']->table('order_action'). " AS a ON a.order_id = o.order_id ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ". $where.
                  " AND a.log_time >='".(strtotime($filter['korea_start_date']))."'".
           " AND  a.log_time < '".(strtotime($filter['korea_end_date'])+60*60*24)."'".
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn,a.log_time,a.shipping_status, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.callback_status, u.home_phone, " .
            "o.pay_status, o.consignee, o.province, o.address, o.email, o.tel, o.extension_code, o.extension_id, " .
         ...
         } elseif ($filter['daohuo'])
        {

        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.province, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.callback_status, u.home_phone, " .
           ...
           // " LEFT JOIN " .$GLOBALS['ecs']->table('order_goods'). " AS g ON g.order_id = o.order_id ".
            ...
            // " AND  g.status = 3 ".
             ...
             "from " .$GLOBALS['ecs']->table('order_goods'). " AS g where g.status != 3 and o.order_id = g.order_id ".
             ")".
            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
             ...
             /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.province, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.callback_status, u.home_phone, " .
            "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, " .
            "(" . order_amount_field('o.') . ") AS total_fee, " .
            "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
...
function get_order_goods($order)
{
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.suppliers_id AS suppliers_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, b.brand_id, b.discount, p.product_sn " .
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT delivery_id, delivery_sn, order_sn, order_id,user_id,invoice_no,send_email, add_time, action_user, consignee, country,
 ...
 /* 取得发货单商品 */
    function get_delivery_goods($delivery_id){
         $sql = 'SELECT *
            FROM ' . $GLOBALS['ecs']->table('suppliers') . '
            WHERE is_check = 1
            ORDER BY suppliers_name ASC';
...
function delivery_email()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['status'] >= 0  && $filter['status'] < 4)
        {
            $where .= " AND d.status = '".$filter['status']."' ";
        }
        if ($filter['status'] == 5)
        {
            $where .= " AND d.status_paizi = 2";
        }
        if ($filter['status'] == 6)
        {
            $where .= " AND d.status_paizi = 2 AND d.status = 0 "  ;
        }
        if ($filter['delivery_sn'])
        {
            $where .= " AND delivery_sn LIKE '%" . mysql_like_quote($filter['delivery_sn']) . "%'";
     ...
     if ($filter['todaydaohuo']==1)
        ...
        $sql = "SELECT delivery_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                       province, city, district, tel, status,status_paizi, update_time, email, suppliers_id
                FROM " . $GLOBALS['ecs']->table("delivery_order") . "
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
            ...
            /* 取得发货单商品 */
             $goods_sql = "SELECT d.* ,o.order_sn,g.goods_thumb ,o.add_time ,  b.brand_name ,b.brand_id , b.discount , o.order_status , o.shipping_status , r.delivery_id 
                  FROM " . $GLOBALS['ecs']->table('order_goods')  . " AS d ".
                 // "LEFT JOIN " . $GLOBALS['ecs']->table('order_action') . ' AS a ON a.order_id = d.order_id ' . 
                  "LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . ' AS o ON o.order_id = d.order_id ' . 
                  "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = d.goods_id ' . 
