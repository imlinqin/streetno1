...
elseif ($_REQUEST['act'] == 'hanbi')
{
    ...
    admin_priv('shop_config');
      ...
      $where = 'WHERE 1 ';
       $where .= 'AND b.huilv = 1';
        ...
        /* 查询 */
        $sql = "SELECT shop_price , goods_id,supply_price " .
           
            " FROM " . $GLOBALS['ecs']->table('goods') . " AS g  " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('brand'). " AS b ON b.brand_id=g.brand_id ". $where .
    ...
    $row = $GLOBALS['db']->getAll($sql);
        ...
        print_r("<pre>");
        // print_r($row);
     $price_rate     = $_CFG['market_price_rate']; // 韩币汇率
     foreach ($row AS $key => $value)
                                {
                                    
                                        
       if($value['supply_price']){

        $shop_price =  ceil($value['supply_price']/$price_rate) ;
                                              
                                                $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('goods') . " SET shop_price = '$shop_price' " .
                                                 "WHERE goods_id = $value[goods_id]");
                                                  print_r("<br/>".$value[goods_id]);

                                          }  
                                          else{
                                                print_r("<br/>该商品韩币为0,id为".$value[goods_id]);
                                          }  
