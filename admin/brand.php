...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $is_show = isset($_REQUEST['is_show']) ? intval($_REQUEST['is_show']) : 0;
     $huilv = isset($_REQUEST['huilv']) ? intval($_REQUEST['huilv']) : 0;
    ...
    if (!empty($_POST['brand_desc_other']))
    {
        $_POST['brand_desc_other'] = $_POST['brand_desc_other'];
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc,brand_desc_other, brand_logo, is_show,huilv, sort_order,discount,promote,email) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]','$_POST[brand_desc_other]', '$img_name', '$is_show', '$huilv', '$_POST[sort_order]','$_POST[discount]','$_POST[promote]','$_POST[email]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id,password_paizi, brand_name, site_url, brand_logo, brand_desc,brand_desc_other, brand_logo, is_show, huilv,sort_order ,discount ,promote,email, end_date ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $param = "brand_name = '$_POST[brand_name]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', brand_desc_other='$_POST[brand_desc_other]',is_show='$is_show', huilv='$huilv', sort_order='$_POST[sort_order]',discount= '$_POST[discount]' ,promote= '$_POST[promote]' ,email= '$_POST[email]', end_date= '$_POST[end_date]',password_paizi= '$_POST[password_paizi]' ";
    if (!empty($img_name))
    {
        //有图片上传
