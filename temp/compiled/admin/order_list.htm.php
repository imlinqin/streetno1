...
<div class="form-div">
  <form action="orderdown.php?act=downorder" method="get" name="downForm">
   
    订单运费表 日期
    <input name="korea_start_date" type="date" value='2018-09-01' size="12" maxlength="10" /> -
    ...
    <input type="hidden" name="act" value="downorder" class="button" />
    <input type="submit" value="下载订单" class="button" />
  ...
  <form action="javascript:searchOrder()" name="searchForm">
    ...
    <input name="korea_end_date" type="date" size="12" value='<?php echo $this->_var['korea_end_date']; ?>' maxlength="10" />
    (按发货日期
    是<input name="fahuo" type="radio"  value='1'  />
    否<input name="fahuo" type="radio" checked value='0'  />
     )
     (商品到货 是
    <input name="daohuo" type="radio" value='1' /> 否
    <input name="daohuo" type="radio" checked value='0' /> )
     <?php echo $this->_var['lang']['all_status']; ?>
    <select name="status" id="status">
      <option value="-1"><?php echo $this->_var['lang']['select_please']; ?></option>
      <?php echo $this->html_options(array('options'=>$this->_var['status_list'],'selected'=>'-1')); ?>
...
?>
  <tr>
    ...
    <td align="left" valign="top"><a href="mailto:<?php echo $this->_var['order']['email']; ?>"> <?php echo htmlspecialchars($this->_var['order']['consignee']); ?></a><?php if ($this->_var['order']['tel']): ?> [TEL: <?php echo htmlspecialchars($this->_var['order']['tel']); ?>]<?php endif; ?> <br /> 
<?php if ($this->_var['order']['province'] >= 33 && $this->_var['order']['province'] <= 36): ?>
    <div style="color:white;background:red"><?php echo htmlspecialchars($this->_var['order']['address']); ?></div>
    <?php else: ?>
<?php echo htmlspecialchars($this->_var['order']['address']); ?>
    <?php endif; ?>
    </td>
...
<?php if ($this->_var['full_page']): ?>
  ...
  <div>
    ...
    <input name="export2" type="submit" id="btnSubmit5" value="导出为EXCEL" class="button" disabled="true" onclick="this.form.target = '_blank'" />
    <input name="export3" type="submit" id="btnSubmit6" value="导出圆通为EXCEL" class="button" disabled="true" onclick="this.form.target = '_blank'"
    />
...
<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    ...
    function searchOrder()
    {
      ...
      listTable.filter['fahuo'] = document.forms['searchForm'].elements['fahuo'].value;
       listTable.filter['daohuo'] = document.forms['searchForm'].elements['daohuo'].value;
        listTable.loadList();
    }
