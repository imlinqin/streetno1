...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['configure'] = array(
                                    ...
                                    array('name' => 'step_fee',     'value'=>0.5),  /* 续重每1000克增加的价格 */
                                );
...
class sf_express
{
    ...
    function calculate($goods_weight, $goods_amount, $goods_number)
    {
           ...
           $sql = 'SELECT home_phone FROM ' . $GLOBALS['ecs']->table('users') . " WHERE user_id = '".$_SESSION['user_id']."'";
          $user_type = $GLOBALS['db']->getOne($sql);
     //$GLOBALS['ecs']->table('users')

                                   // echo $user_type;
        if ($this->configure['free_money'] > 0 && $goods_amount >= $this->configure['free_money'] && $user_type !=2)
        {
            return 0;
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $fee = -floor(-($goods_weight * 10 / 5)) * $this->configure['step_fee'] ;
                   // echo $goods_weight ;
                  // $fee +=  ($goods_weight % 5) != 0  ? 30 :0 ;
                   // echo -floor(-($goods_weight * 10 / 5));
                }
            }
