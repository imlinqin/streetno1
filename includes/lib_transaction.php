...
function get_profile($user_id)
{
    ...
    if ($bonus)
    {
        ...
        {
            $bonus[$i]['type_money'] = price_format($bonus[$i]['type_money'], true);
        }
    }
    ...
    $info['user_money']  = price_format($info['user_money'], true);
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, consignee, order_sn, order_status, shipping_status, pay_status, add_time, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $sqla = "SELECT * FROM " . $GLOBALS['ecs']->table('order_goods') .
        " WHERE order_id = '$order_id'";

        $resa=  $GLOBALS['db']->query($sqla);
       
        
        $goodstatus = '(全发货)';
        while ($rowa = $GLOBALS['db']->fetchRow($resa))
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'total_fee'      => price_format($row['total_fee'], true),
                       'consignee'       => $row['consignee'],
                       'goodstatus'       => $row['shipping_status'] ==1 ? $goodstatus :'',
                       'handler'        => $row['handler']);
    }
 ...
 * @return  array       $order_list     订单列表
 */
function get_user_orders2($user_id, $num = 10, $start = 0,$consignee)
{
    /* 取得订单列表 */
    $arr    = array();
      $where = 'WHERE 1 ';
      if($consignee){
         $where .= " AND consignee LIKE '%" . mysql_like_quote($consignee) . "%'";
      }
    $sql = "SELECT order_id, consignee, order_sn, order_status, shipping_status, pay_status, add_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount - goods_discount_fee) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           " $where AND user_id = '$user_id' ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
