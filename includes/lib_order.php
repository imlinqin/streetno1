 ...
 * @return  float   运费
 */
function shipping_fee($shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='',$consignee = '')
{       

       // print_r ($consignee['province']);
       //香港
      if($consignee && $consignee['province'] == 34){
            if( $goods_weight  <= 1){
                    return 73;
            }
        
            else if ($goods_weight  <= 1.5){
                return 76;
            }

            else if ($goods_weight  <= 2){
                return 80;
            }
            else if ($goods_weight  <= 2.5){
                return 83;
            }
            else if ($goods_weight  <= 3){
                return 87;
            }
            else {
                return 87;
        ...
        //韩国
        else if ($consignee && $consignee['province'] == 36 && $consignee['city'] == 4047 ) {
             if(  $goods_number <= 2){
                    return 17;
            }
        
            else if ($goods_number <= 5 ){
                return 25;
            }
            else {
                return 30;
        ...
        //国内
            ...
            else if ($goods_weight  <= 3.5){
                return 155;
            }
            else if ($goods_weight  <= 4){
                return 176;
            }
             else if ($goods_weight  <= 4.5){
                return 198;
            }
            else if ($goods_weight  <= 5){
                return 219;
            }
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        $order['formated_goods_amount']   = price_format($order['goods_amount'], true);
        $order['formated_discount']       = price_format($order['discount'], true);
        $order['formated_tax']            = price_format($order['tax'], true);
        $order['formated_shipping_fee']   = price_format($order['shipping_fee'], true);
        $order['formated_insure_fee']     = price_format($order['insure_fee'], true);
        $order['formated_pay_fee']        = price_format($order['pay_fee'], true);
        $order['formated_pack_fee']       = price_format($order['pack_fee'], true);
        $order['formated_card_fee']       = price_format($order['card_fee'], true);
        $order['formated_total_fee']      = price_format($order['total_fee'], true);
        $order['formated_money_paid']     = price_format($order['money_paid'], true);
        $order['formated_bonus']          = price_format($order['bonus'], true);
        $order['formated_integral_money'] = price_format($order['integral_money'], true);
        $order['formated_surplus']        = price_format($order['surplus'], true);
        $order['formated_order_amount']   = price_format(abs($order['order_amount']), true);
        $order['formated_add_time']       = local_date($GLOBALS['_CFG']['time_format'], $order['add_time']);
        $order['formated_goods_discount_fee']   = price_format($order['goods_discount_fee'], true);
    }
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            "goods_price, goods_attr, is_real, parent_id, is_gift, status, " .
...
function order_fee($order, $goods, $consignee)
{
    ...
    $total['discount_formated'] = price_format($total['discount'], true);
    ...
    $total['tax_formated'] = price_format($total['tax'], true);
    ...
    $total['pack_fee_formated'] = price_format($total['pack_fee'], true);
    ...
    $total['card_fee_formated'] = price_format($total['card_fee'], true);
    ...
    $total['bonus_formated'] = price_format($total['bonus'], true);
     ...
     if (!empty($order['bonus_kill']))
    {
        ...
        $total['bonus_kill_formated'] = price_format($total['bonus_kill'], true);
    }


     // 获取会员等级 批发 会员费用 为0  by lin
      $usersql = 'SELECT r.discount FROM ' . $GLOBALS['ecs']->table('users'). " AS u " .
                 "LEFT JOIN " .$GLOBALS['ecs']->table('user_rank') . " AS r ON r.rank_id = u.user_rank " .
                                " WHERE user_id = '".$_SESSION['user_id']."'";
    $user_rank = $GLOBALS['db']->getOne($usersql);


     $usersql = 'SELECT u.home_phone FROM ' . $GLOBALS['ecs']->table('users'). " AS u " .
                 "LEFT JOIN " . $GLOBALS['ecs']->table('user_rank') . " AS r ON r.rank_id = u.user_rank " .
                                " WHERE user_id = '".$_SESSION['user_id']."'";
                              

    $user_type = $GLOBALS['db']->getOne($usersql);
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            // 运费修改 bylin 国内运费
            $total['shipping_fee'] = ($shipping_count == 0 AND $weight_price['free_shipping'] == 1) ?0 : $user_type !=1 ?  shipping_fee($shipping_info['shipping_code'],$shipping_info['configure'], $weight_price['weight'], $total['goods_price'], $weight_price['number'],$consignee) : 0;
    ...
    $total['shipping_fee_formated']    = price_format($total['shipping_fee'], true);
    $total['shipping_insure_formated'] = price_format($total['shipping_insure'], true);
    ...
    $total['surplus_formated'] = price_format($order['surplus'], true);
    ...
    $total['amount_formated']  = price_format($total['amount'], true);
    ...
    $total['will_get_bonus']        = $order['extension_code'] == 'exchange_goods' ? 0 : price_format(get_total_bonus(), true);
    $total['formated_goods_price']  = price_format($total['goods_price'], true);
    $total['formated_market_price'] = price_format($total['market_price'], true);
    $total['formated_saving']       = price_format($total['saving'], true);
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    {
        $arr[$key]['formated_market_price'] = price_format($value['market_price'], true);
        $arr[$key]['formated_goods_price']  = price_format($value['goods_price'], true);
        $arr[$key]['formated_subtotal']     = price_format($value['subtotal'], true);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $rec_type = CART_GENERAL_GOODS)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT b.promote, b.discount,b.end_date, g.goods_name, g.brand_discount, g.goods_sn, g.is_on_sale, g.is_real, ".
                "g.market_price, g.shop_price AS org_price, g.promote_price, g.promote_start_date, ".
                "g.promote_end_date,g.brand_discount_end_date, g.goods_weight, g.integral, g.extension_code, ".
    ...
    $goods['promote'] = $goods['end_date'] >= $now_date ? $goods['promote'] : 0;
    $promote = $goods['market_price'] < 1 && $goods['promote_end_date'] >= strtotime($now_date) ? $goods['market_price'] : $goods['promote'];
    $promote = $goods['market_price'] == 1 ? 0 : $promote;
    $discount = $goods['brand_discount'] && $goods['brand_discount_end_date'] >=$now_date ? $goods['brand_discount'] : $goods['discount'] ;  
    /* 初始化要插入购物车的基本件数据 */
    $parent = array(
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage)
            {
                ...
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$num'" .
                       " , goods_price = '$goods_price' , promote = '123' ".
                       " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                       " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                       " AND extension_code <> 'package_buy' " .
                       "AND rec_type = 'CART_GENERAL_GOODS'";
...
function get_cart_goods($rec_type = CART_GENERAL_GOODS)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['subtotal']     = price_format($row['goods_price'] * $row['goods_number'], true);
        $row['goods_price']  = price_format($row['goods_price'], true);
        $row['market_price'] = price_format($row['market_price'], true);
