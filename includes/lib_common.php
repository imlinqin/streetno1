...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo,b.promote, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    //取得商品促销价格列表
    /* 取得商品信息 */
    // $sql = "SELECT g.promote_price, g.promote_start_date, g.market_price, g.promote_end_date, ".
    ...
    $sql = "SELECT g.promote_price, g.promote_start_date, g.market_price, g.promote_end_date, b.end_date, ".
        "IFNULL(mp.user_price, g.shop_price * '" . $_SESSION['discount'] . "') AS shop_price ,".
        "IFNULL(mp.user_price, g.shop_price * b.promote /100 * '" . $_SESSION['discount'] . "') AS brand_promote_price ,". //牌子 價格 = 會員價 * 牌子折扣 by lin
        "IFNULL(mp.user_price, g.shop_price * g.market_price  * '" . $_SESSION['discount'] . "') AS good_price ". //商品折扣 = 會員價 * 牌子折扣 by lin
        " FROM " .$GLOBALS['ecs']->table('goods'). " AS g ".
        " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '" . $_SESSION['user_rank']. "' ".
    ...
    // print_r($goods['end_date']);
     /* 计算商品的促销价格 */
     if ($goods['market_price'] == 1 || ($goods['market_price'] < 1.00 &&  $goods['promote_end_date'] >= strtotime($now_date))  ){
        $brand_promote_price = $goods['good_price'];  
        

     }
   elseif ($goods['brand_promote_price'] > 0 && strtotime($goods['end_date']) >= strtotime($now_date) )
    {
        $brand_promote_price = $goods['brand_promote_price'] ;  
    }
    else
    {
        $brand_promote_price = 0;
    }
    ...
    //比较商品的促销价格，会员价格，优惠价格
    if (empty($volume_price) && empty($promote_price) && empty($brand_promote_price))
    {
        //如果优惠价格，促销价格都为空则取会员价格
        $final_price = $user_price;
    }
     elseif (!empty($brand_promote_price))
    {
        //如果品牌有优惠优先显示。  by lin
        $final_price = $brand_promote_price;
    }
