    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
				...
				$cat_arr[$row['cat_id']]['cnname'] = explode('|',$row['cat_name'])[0];
				$cat_arr[$row['cat_id']]['enname'] = explode('|',$row['cat_name'])[1] ? explode('|',$row['cat_name'])[1] : explode('|',$row['cat_name'])[0] ;
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order,b.promote ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
               ' ORDER BY g.sort_order, g.add_time DESC';
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, b.promote, b.end_date, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['brand_shop_price'] = $shop_price;
            $goods[$idx]['good_price'] = $row['market_price'] < 1.00 ?  price_format($row['good_price']) :  '';
