...
class sms
{
    ...
    function send($phones,$msg,$send_date = '', $send_num = 1,$sms_type='',$version='1.0')
    {
        ...
        $api_url = "http://api.smsbao.com/sms?";

       
		foreach($contents as $key=>$val){
			$send_str['m']= trim($val['phones']);
			$send_str['c']=$val['content'];
			$send_str['u']=$GLOBALS['_CFG']['短信宝账号'];
			$send_str['p']=MD5($GLOBALS['_CFG']['短信宝密码']);
			$rall = file_get_contents($api_url.http_build_query($send_str));
			//$result = $rall->parsed();
