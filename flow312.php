...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    $usersql = 'SELECT u.home_phone FROM ' . $ecs->table('users'). " AS u " .
                 "LEFT JOIN " . $ecs->table('user_rank') . " AS r ON r.rank_id = u.user_rank " .
                                " WHERE user_id = '".$_SESSION['user_id']."'";
                              

    $user_type = $db->getOne($usersql);
    ...
    // 批发 快递 费用为0 ；
    //$total['shipping_fee_formated'] = $user_type['home_phone'] != 1 ?   $user_type['home_phone'] : 0;
    ...
    foreach ($shipping_list AS $key => $val)
    {
        ...
        $shipping_fee = ($shipping_count == 0 AND $cart_weight_price['free_shipping'] == 1) ? 0 : $user_type['home_phone'] != 1 ?shipping_fee($val['shipping_code'], unserialize($val['configure']),
        $cart_weight_price['weight'], $cart_weight_price['amount'], $cart_weight_price['number'],$consignee) : 0;

        
       // $hongkong_fee = 0;
        
    //     // 判断香港邮费
        $shipping_list[$key]['format_shipping_fee'] = price_format($shipping_fee, true);
    //    // echo $consignee['province'];
    //     if($consignee['province'] == 34){
    //          if( $cart_weight_price['weight']<= 1){
    //         $shipping_list[$key]['format_shipping_fee'] = price_format(73, true);
    //     }
    //     else if ($cart_weight_price['weight']<= 1.5) {
    //         $shipping_list[$key]['format_shipping_fee'] = price_format(76, true);
    //     }
    //     else if ($cart_weight_price['weight']<= 2){
    //         $shipping_list[$key]['format_shipping_fee'] = price_format(80, true);
    //     }
    //     else if ($cart_weight_price['weight']<= 2.5){
    //         $shipping_list[$key]['format_shipping_fee'] = price_format(83, true);
    //     }
    //     else if ($cart_weight_price['weight']<= 3){
    //         $shipping_list[$key]['format_shipping_fee'] = price_format(87, true);
    //     }
    //     else {
    //         // $shipping_list[$key]['format_shipping_fee'] = price_format(100, true);
    //     }

    //     }
        // total 费用总计
       $total['shipping_fee_formated'] = $shipping_list[$key]['format_shipping_fee']  ;
            $smarty->assign('total', $total);

        $shipping_list[$key]['shipping_fee']        = $shipping_fee;
        $shipping_list[$key]['free_money']          = price_format($shipping_cfg['free_money'], true);
...
//$order['shipping_fee'] = $total['shipping_fee'];
    ...
    // if($consignee['province'] == 34){
    //          if( $cart_weight_price['weight']<= 1){
    //              $total['shipping_fee'] = 73;
    //     }
    //     else if ($cart_weight_price['weight']<= 1.5) {
    //         $total['shipping_fee'] = 76;
    //     }
    //     else if ($cart_weight_price['weight']<= 2){
    //         $total['shipping_fee'] = 80;
    //     }
    //     else if ($cart_weight_price['weight']<= 2.5){
    //         $total['shipping_fee'] = 83;
    //     }
    //     else if ($cart_weight_price['weight']<= 3){
    //         $total['shipping_fee'] = 87;
    //     }

    //     }

    $order['shipping_fee'] = $user_type['home_phone'] != 1 ?    $total['shipping_fee'] : 0;
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                ...
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, promote, discount) ".
