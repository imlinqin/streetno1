...
$pinyin = array(
  array('name'=>'b','age'=>23),
  array('name'=>'abc','age'=>4),
  array('name'=>'andyw','age'=>24),
...
$charArray = array();
$pinyin = get_brands();
 foreach($pinyin as $key => $name){  
     
            $char =zhcnToPinyin($name['brand_name']);  
             //print_r($pinyin[$key]);
            ...
            array_push($nameArray,$name['brand_name']);  

            $charArray[$char] =  $charArray[$char] ? $charArray[$char] : array(); 

             array_push($charArray[$char],$pinyin[$key]);   
        } 

//array_multisort($sort, SORT_ASC, $charArray);
//var_dump($pinyin);
//print_r($charArray);
ksort($charArray);
//print_r(array_keys($charArray));


        $smarty->assign('brand_list',$charArray );
        $smarty->assign('brand_zimu',array_keys($charArray) );
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, b.promote,b.end_date, ' .
